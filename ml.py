...
def get_class_stats(predicted_classes, y):
    ...
    return {'ACC': float(acc), 'F1Score': float(f1_sc), 'Ck': float(cohen_kappa),
            'MCC': float(matthews_corr), 'Precision': float(precision), 'Recall': float(recall), 'Specificity': float(specificity)}
